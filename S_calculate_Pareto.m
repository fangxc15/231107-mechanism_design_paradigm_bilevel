...
% Contributors: Xichen Fang


clear 
script_type = 'xls';
Setting.new_version = 11;
version_suffix = '_newversion11'; %决定了到底采用哪个IC的定义，不要改动

Result_root_folder = ['Result',version_suffix];
mkdir(Result_root_folder);
...
[Setting,Para] = IEEE_30_Setting(Setting,xlsname,sheetname);
...
% 找到满足BB的一个合适的点，作为solve_vertex舍SW保另外三项的输入项
...
%% 极点的数据整理
vertex = get_vertex_data(Result_vertex, Result_maxSW, Para, Setting, Num);
...
save([Result_root_folder ,'/',save_name]);
%%
Picture_folder = [Picture_root_folder,'/',save_name];
mkdir(Picture_folder);

plot_Pareto_fun(1, Para, Result_ParetoBBSW,...

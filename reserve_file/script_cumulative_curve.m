...
for choose_spread = -5:0.1:10
    spread_num = round((choose_spread+5)/0.1+1)
    ...
    %             input_Gvector = 
    %         else
    %             input_Gvector = temp_cumu(temp_Gset,p+G_setoff);
    %         end
...
%     fprintf('主体获得的效用总额') %Budget Balance
...
for choose_spread = -5:0.1:10
     ...
     Exp_q = result_collect_spread(spread_num).Exp_q;
     rip = result_collect_spread(spread_num).rip;
    ...
    % yn表示有整体发生次数为n的概率, xn表示除去该主体只有发生次数为n的概率
    % y6 = p*x5
    % y5 = p*x4 + (1-p)x5
    % y4 = p*x3 + (1-p)x4;
    % 

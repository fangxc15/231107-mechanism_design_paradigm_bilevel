% save_folder = 'xls_Clearing_V1_8asymmetry_all_4P';
save_folder = 'bilateral_all_200P';
% mkdir('Picture');
mkdir(['Picture/',save_folder]);
%% 画出BB,SW的帕累托前沿(1)
solveindex = find([solution_ParetoBBSW.problem] == 0);
figure(1)
...
figure(4)
matrix_plot = sortrows([-[Result_ParetoBBSW(solveindex).surplus]
Result_ParetoBBSW(1).welfare-[Result_ParetoBBSW(solveindex).welfare]]',1);
plot(matrix_plot(:,1),matrix_plot(:,2));
xlabel('收支平衡松弛量');
ylabel('社会福利松弛量');
title('关于社会福利和收支平衡松弛量的帕累托曲线')
...
saveas(5,['Picture/',save_folder,'/','BB_IC_Pareto.jpg'])
